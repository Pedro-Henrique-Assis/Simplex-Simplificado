  ...
  return (
    <div className="container-page py-12 md:py-16">
      <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
        ...
        <div className="flex flex-wrap gap-2" role="group" aria-label="Filtrar por dificuldade">{filters.map(([label, value]) => <button key={label} onClick={() => setFilter(value)} className={`rounded-lg border px-3 py-2 text-sm font-semibold ${filter === value ? "border-black bg-black text-white" : "border-black/20"}`}>{label}</button>)}</div>
      </div>
